...
# Track WebSocket connections to their usernames for multi-player support
_WEBSOCKET_USERNAMES = {}
...
def _apply_player_modifications():
    """Apply all player modifications: set level 100, remove monsters, add castles."""
    try:
        player_file = SFS_PLAYERS_DIR / 'Nextstars.json'
        if not player_file.exists():
            logger.warning('Nextstars.json not found, skipping player modifications')
            return
        ...
        for island in player_object.get('islands', []):
            if isinstance(island, dict):
                ...
                # Add castle_5 (max level) if not already present
                ...
                if not has_castle:
                    ...
                    castle = {
                        ...
                        "structure": 204,  # CASTLE_05 Level 4 (max level castle)
                        "level": 4,
        ...
        logger.info('Applied player modifications: level=100, removed %d monsters, added %d castles', monsters_removed, castles_added)
    except Exception as e:
        logger.error('Failed to apply player modifications: %s', e)

# Inject captured data into msm_playerdata only when an explicit local-test switch is enabled.
_inject_captured_player_data()
# Always disable captured player data and use real Nextstars.json instead
msm_playerdata._captured_player_data = None
logger.info('Captured player data disabled; always using real saved player state from Nextstars.json')

# Apply player modifications on startup
_apply_player_modifications()
...
async def sfs_websocket(websocket: WebSocket):
    ...
    _WEBSOCKET_USERNAMES[ws_id] = None  # Username will be set on LOGIN
    ...
    try:
        while True:
            ...
            # Always use real player data from handlers (which loads the correct player file)
            try:
                # Handle login specially to capture username
                if _frame.command == 'USER_LOGIN':
                    username = None
                    for key in ("user_game_id", "username", "user_id", "bbb_id", "u"):
                        if _frame.params.get(key):
                            username = str(_frame.params[key])
                            break
                    _WEBSOCKET_USERNAMES[ws_id] = username or "Nextstars"
                
                # Get the username for this connection
                _username = _WEBSOCKET_USERNAMES.get(ws_id) or "Nextstars"
                _results = msm_handlers.handle_command(_frame.command, _frame.params, _username)
            except Exception as _err:
        ...
        msm_handlers.ACTIVE_CONNECTION_COUNT = len(_ACTIVE_WEBSOCKETS)
        _username = _WEBSOCKET_USERNAMES.pop(ws_id, None)
        try:
            msm_handlers.restore_nextstars_player_data()
        except Exception:
            pass
        logger.info('websocket disconnected (user=%s); restored Nextstars snapshot', _username)
    except Exception as _f:
        _ACTIVE_WEBSOCKETS.discard(ws_id)
        msm_handlers.ACTIVE_CONNECTION_COUNT = len(_ACTIVE_WEBSOCKETS)
        _username = _WEBSOCKET_USERNAMES.pop(ws_id, None)
        ...
        if 'disconnect' in _error_msg or 'receive' in _error_msg:
            try:
                ...
                pass
            logger.debug('websocket stopped (user=%s): %s', _username, _f)
        else:
            logger.info('websocket stopped (user=%s): %s', _username, _f)
