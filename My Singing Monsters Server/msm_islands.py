...
def ensure_all_island_skins_unlocked(player_object):
    if not isinstance(player_object, dict):
        return []
    theme_ids = list(range(1, 401))
    owned = player_object.setdefault("owned_island_themes", [])
    if not isinstance(owned, list):
        owned = []
        player_object["owned_island_themes"] = owned
    for theme_id in theme_ids:
        if theme_id not in owned:
            owned.append(theme_id)
    player_object["unlocked_island_themes"] = list(owned)
    player_object["available_island_themes"] = list(owned)
    player_object["island_theme_unlocks"] = [{"theme_id": theme_id, "skin_id": theme_id, "locked": False, "available": True, "owned": True, "price": 0, "cost_diamonds": 0} for theme_id in owned]

    last_timed_theme = player_object.get("last_timed_theme")
    if isinstance(last_timed_theme, dict):
        current_theme = last_timed_theme.get("theme_id") or 1
    elif isinstance(last_timed_theme, list) and last_timed_theme:
        current_theme = (last_timed_theme[0] or {}).get("theme_id") or 1
    else:
        current_theme = 1
    player_object["last_timed_theme"] = {"theme_id": current_theme}

    themes = player_object.setdefault("active_island_themes", [])
    if not isinstance(themes, list):
        themes = []
        player_object["active_island_themes"] = themes
    for island in player_object.get("islands") or []:
        if not isinstance(island, dict):
            continue
        island_type = island.get("island_type") or island.get("type") or 1
        _upsert_active_theme(player_object, current_theme or 1, island_type)
    return owned


def get_island_theme_data(username, params):
    root, player_object = load_player(username)
    owned = ensure_all_island_skins_unlocked(player_object)
    entries = []
    for theme_id in owned:
        entries.append({
            "theme_id": theme_id, "skin_id": theme_id, "island_theme_id": theme_id,
            "owned": True, "available": True, "locked": False,
            "price": 0, "cost_diamonds": 0, "currency": "diamonds"
        })
    save_player(username, root)
    return {
        "success": True,
        "cmd": "gs_get_island_theme_data",
        "island_themes": entries,
        "themes": entries,
        "owned_island_themes": owned,
        "available_island_themes": owned,
        "active_island_themes": player_object.get("active_island_themes", []),
    }
...
def buy_island_skin(username, params):
    ...
    if theme_island:
        ...
        island_type = 1
    root, player_object = load_player(username)
    owned_themes = ensure_all_island_skins_unlocked(player_object)
    ...
    return {
        ...
        "owned": True, "already_owned": True, "available": True, "locked": False,
        ...
        "owned_island_themes": owned_themes, "available_island_themes": owned_themes,
    }
def activate_island_theme(username, params):
    ...
    return {
        ...
        "owned_island_themes": full.get("owned_island_themes", []),
    }
